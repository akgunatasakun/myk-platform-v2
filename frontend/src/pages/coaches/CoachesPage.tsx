/**
 * /antrenorler — Antrenör listesi
 *
 * Backend: GET /api/v1/persons?role_code=antrenor
 */
...
export default function CoachesPage() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      setSkip(0)
      fetchCoaches(search, activeFilter, 0)
    }, 300)

    return () => {
      if (debounceRef.current) clearTimeout(debounceRef.current)
    }
  }, [search, activeFilter, fetchCoaches])

  useEffect(() => {
    fetchCoaches(search, activeFilter, skip)
  ...
  const otherRoles = (coach: Person) =>
    coach.role_codes
      .filter((r) => r !== 'antrenor')
      .map((r) => ROLE_LABELS[r] ?? r)

  return (
    <AppShell title="Antrenörler">
      <div className="page-header">
        <h1 className="page-title">
          Antrenörler{data && data.total > 0 && ` (${data.total})`}
        </h1>
        <button
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-state-desc">
              {search || activeFilter
                ? 'Arama kriterlerinize uyan antrenör bulunamadı.'
            ...
            <table className="data-table">
              ...
              <tbody>
                {data.items.map((coach) => {
                  const others = otherRoles(coach)
                  return (
                    <tr
                      key={coach.id}
                      onClick={() => navigate(`/antrenorler/${coach.id}`)}
                    ...
                    >
                      ...
                      <td>{coach.phone ?? '—'}</td>
                      <td style={{ fontSize: 13 }}>{coach.email ?? '—'}</td>
                      <td>
                        {others.length > 0 ? (
