#!/usr/bin/env python3
"""DMS Dry-Run Import Script — Belge Manifesti Analiz Aracı.
...
Çıktı:
    import_plan.json — özet + belge planı + eşleşmeyen satırlar
...
def run(manifest_path: str, source_dir: str, output_path: str) -> None:
    ...
    if src_dir.exists():
        ...
        print(f"[UYARI] source-dir bulunamadı: {src_dir} — disk eşleştirmesi atlanıyor")
    ...
    for i, row in enumerate(rows):
        # Olası kolon adları (manifeste formatına göre)
        filename = (
            row.get("dosya_adi") or row.get("filename") or row.get("file_name") or ""
        ).strip()
        content_status_raw = (
            row.get("icerik_durumu") or row.get("içerik_durumu") or row.get("content_status") or ""
        ).strip()
        conflict_val = (
            row.get("celiski_durumu") or row.get("çelişki_durumu") or row.get("conflict") or ""
        ).strip()
        doc_code = (
        ...
        doc_type = (
            row.get("belge_turu") or row.get("belge_türü") or row.get("document_type") or "diger"
        ).strip()
        ...
        if filename and src_dir.exists():
            # Doğrudan dosya adı veya alt klasörlerde ara
            candidates = [p for p in disk_files if Path(p).name == filename or p == filename]
            if candidates:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DMS dry-run import — manifesti analiz et, DB'ye yazmaz."
    )
    parser.add_argument("--manifest", required=True, help="CSV manifest dosyası yolu")
    parser.add_argument("--source-dir", required=True, help="Fiziksel dosyaların bulunduğu klasör")
    parser.add_argument(
        "--output", default="import_plan.json", help="Çıktı JSON dosyası (varsayılan: import_plan.json)"
    )
